    ...
    while (1) {
        ...
        if (chars_seen == 0 && (int32_t)(now - silence_deadline) >= 0) {
            if (silence_stage == 0) {
                ...
                ESP_LOGE(TAG, "GY-GPS6MV2 continua sin enviar datos tras recuperar UART; se solicita reinicio controlado");
                silence_stage = 2;
            }
...
esp_err_t gps_init(void) {
    ...
    esp_err_t err = uart_driver_install(GPS_UART_PORT, GPS_UART_BUF_SIZE, 0, 0, NULL, 0);
    if (err != ESP_OK) {
    ...
    s_recovery_count = 0;
    s_restart_requested = false;
    ESP_LOGI(TAG, "GY-GPS6MV2 inicializado; se esperara primer fix valido antes de medir");
    return ESP_OK;
}
